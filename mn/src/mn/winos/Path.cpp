...
namespace mn
{
	...
	path_current(Allocator allocator)
	{
		...
		[[maybe_unused]] DWORD written_size =
			GetCurrentDirectory((DWORD)(os_str.size / sizeof(TCHAR)), (LPWSTR)os_str.ptr);
	...
	path_absolute(const char *path, Allocator allocator)
	{
		...
		[[maybe_unused]] DWORD written_size =
			GetFullPathName((LPCWSTR)os_path.ptr, required_size, (LPWSTR)full_path.ptr, NULL);
	...
	path_entries(const char *path, Allocator allocator)
	{
		...
		if (search_handle != INVALID_HANDLE_VALUE)
		{
			while (search_handle != INVALID_HANDLE_VALUE)
			{
				...
				entry.name = from_os_encoding(
					Block{(void *)file_data.cFileName, (_tcsclen(file_data.cFileName) + 1) * sizeof(TCHAR)},
					allocator);
